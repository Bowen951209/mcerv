...
impl CommandManager {
    ...
    fn list_mods_handler(state: &mut State, tokens: &[String]) -> anyhow::Result<(), String> {
        ...
        let update_accepted = if !available_updates.is_empty() {
            println!("Do you want to apply these updates? (y/n)");
            state
                .editor
                .readline("> ")
        ...
        let should_update = update_accepted || tokens.contains(&"--update".to_string());
        if should_update {
    ...
    fn add_server_handler(state: &mut State, tokens: &[String]) -> Result<(), String> {
        ...
        // Ask user if they agree to eula
        println!(
            "Do you agree to Minecraft server EULA? Please ensure you have read and understood the EULA at: https://aka.ms/MinecraftEULA (y/n)"
        );

        let eula_agreed = state
            .editor
            .readline("> ")
            .map_err(|e| format!("Failed to read line to EULA agreement. Error: {e}"))?
            .trim()
            .eq_ignore_ascii_case("y");
        ...
        if eula_agreed {
            Self::accept_eula(server_name).map_err(|e| format!("Failed to accept EULA: {e}"))?;
        }
    ...
    fn accept_eula_handler(state: &mut State, _: &[String]) -> Result<(), String> {
        ...
        Self::accept_eula(&selected_server.name)
            .map_err(|e| format!("Failed to accept EULA: {e}"))?;
    ...
    fn accept_eula(selected_server_name: &str) -> std::io::Result<()> {
        let eula_path: PathBuf = format!("instances/{selected_server_name}/eula.txt").into();

        fs::create_dir_all(eula_path.parent().unwrap())?;

        let content = r#"# This file is generated by multi-server and is generated because the user agreed to the Minecraft EULA (https://aka.ms/MinecraftEULA).
eula=true
"#;
